...
class ConvolutionalSelfAttention(nn.Module):
    def __init__(self,
        ...
        self.spatial_H, self.spatial_W, self.spatial_C = None, None, None
        self.spatial_C = n_channels
    ...
    def compute_input_shape(self, batch):
        self.spatial_C, self.spatial_H, self.spatial_W = batch.shape[1:]
        self.input_padder = self.compute_padding(self.padding_type)
        self.spatial_H += 2 * self.padding_tuple[2]
        self.spatial_W += 2 * self.padding_tuple[0]
        self.spatial_H, self.spatial_W = int(self.spatial_H), int(self.spatial_W)
    ...
    def forward(self, batch):
        ...
        """
        if self.spatial_H is None:
            self.compute_input_shape(batch)
            self.create_masks()
...
class ConvAttnWrapper(nn.Module):
    def __init__(self, backbone, variant_kwargs):
        ...
        self.backbone_layers, self.layer_channels = self.backbone.get_network_structure()
    ...
    def inject_variant(self):
        ...
        for injection_instruction in injection_instructions:
            ...
            layer_channels = self.layer_channels[inject_layer-1]
            ...
            for i in range(inject_number):
                ...
                variant_bn = nn.BatchNorm2d(layer_channels)
